...
export default function ContactPage() {
  ...
  // ==========================================
  // Automatically format the phone number (add a hyphen -).
  // ==========================================
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    let val = e.target.value.replace(/\D/g, ""); // ลบทุกอย่างที่ไม่ใช่ตัวเลขออก
    
    // จัดรูปแบบให้เป็น 0XX-XXX-XXXX
  ...
  // ==========================================
  // Manage the Submit button, run the database + email.
  // ==========================================
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call server Action for save in DB and send Email
      ...
      if (result.success) {
        ...
        alert(result.error || "ขออภัย เกิดข้อผิดพลาดทางเซิร์ฟเวอร์ กรุณาลองใหม่อีกครั้ง");
      }
    } catch (error) {
      alert("ระบบขัดข้อง กรุณาติดต่อเราผ่าน LINE แทนชั่วคราวนะครับ");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="font-thai leading-[1.7] bg-white dark:bg-brand-dark text-gray-900 dark:text-gray-100 transition-colors duration-300 min-h-screen">
                  ...
                  <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
                    ...
                    <FormField label="เบอร์โทรศัพท์ *" error={showError("phone") ? errorMsg.phone : null} hint={touched.phone && validate.phone(form.phone) ? "✓ เบอร์โทรถูกต้อง" : null}>
                      <input 
                        ...
                        className={`${inputBaseClasses} ${showError("phone") ? "!border-red-600" : ""}`} 
                        maxLength={12} 
                      />
                    </FormField>
